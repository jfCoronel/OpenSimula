...
class File_met(Component):
    ...
    def get_instant_values(self, datetime):
        ...
        sky_temperature = self.sky_temperature[i] * \
            (1-f)+self.sky_temperature[j]*f
        sol_dir = self.sol_dir[i]*(1-f)+self.sol_dir[j]*f
        sol_dif = self.sol_dif[i]*(1-f)+self.sol_dif[j]*f
    ...
    def _solar_hour_(self, datetime):  # Hora solar desde el principio del año
        ...
        daylight_saving = (hours - (datetime.timestamp() -
                           dt.datetime(datetime.year, 1, 1).timestamp())/3600) + 1
        # Ecuación del tiempo en minutos Duffie and Beckmann
        B = math.radians((day - 1)*360/365)
        ecuacion_tiempo = 229.2*(0.000075+0.001868*math.cos(B)-0.032077 *
                                 math.sin(B)-0.014615*math.cos(2*B)-0.04089*math.sin(2*B))
