...
class Test_component(Component):
    ...
    def __init__(self, name, project):
        ...
        self.add_parameter(Parameter_string_list("string_list", ["Hello World 1", "Hello World 2"]))
        self.add_parameter(Parameter_int_list("int_list", [50, 100], "h"))
        self.add_parameter(Parameter_float_list("float_list", [0.1, 0.2], "m"))
        self.add_parameter(Parameter_options_list("options_list", ["One", "Two"], ["One", "Two", "Three"]))
        self.add_parameter(Parameter_component_list("component_list", ["not_defined", "not_defined"]))
    ...
    def pre_iteration(self, time_index, date):
        ...
        self.variable("t").values[time_index] = (date - self._initial_date_).total_seconds()

        variables_dic = {}
        for key, var in self._variables_.items():
            variables_dic[key] = var.values[time_index]

        self.variable("exp_1").values[time_index] = self.parameter("math_exp").evaluate(variables_dic)
