...
class Exterior_surface(Surface):
    ...
    def _calculate_K(self):
        if (self.parameter("virtual").value):
            ...
            self._k_0 = self.net_area()*(a_0 - self.parameter("h_cv").value[0] -
                                         self._H_RD * self.alpha_lw(0))
            self._k_1 = self.net_area()*(a_1 - self.parameter("h_cv").value[1])
            self._k_01 = self.net_area()*a_01
            self._K = self._k_1 - self._k_01 * self._k_01 / self._k_0
