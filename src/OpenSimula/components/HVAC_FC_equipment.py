...
class HVAC_FC_equipment(Component):
    ...
    def calculate_nominal_effectiveness(self):
        ...
        if self._nominal_total_cooling_capacity > 0:
            T_ri =sicro.GetTDewPointFromTWetBulb(self._T_idb_CN,self._T_iwb_CN,self.props["ATM_PRESSURE"])
    ...
    def calculate_ADP(self,T_i,w_i,T_o,w_o):
        def func(x):
            w_adp = sicro.GetHumRatioFromRelHum(x,1,self.props["ATM_PRESSURE"])
    ...
    def get_cooling_state(self,T_idb,T_iwb,T_iw,F_air, F_water,F_load):
        ...
        """
        if self._nominal_total_cooling_capacity > 0:
            T_ri =sicro.GetTDewPointFromTWetBulb(T_idb,T_iwb,self.props["ATM_PRESSURE"])
            ...
            if T_ri < T_iw: # Dry coil
                ...
                Q_tot = epsilon * self._nominal_air_flow*self.props["RHO_A"] * (h_ia - h_iw)
                Q_sen = Q_tot
            else:  # Wet coil
                h_iw = sicro.GetMoistAirEnthalpy(T_iw,sicro.GetHumRatioFromRelHum(T_iw,1,self.props["ATM_PRESSURE"]))
                Q_tot = epsilon * self._nominal_air_flow*self.props["RHO_A"] * (h_ia - h_iw)
                h_oa = h_ia - Q_tot/(self._nominal_air_flow*self.props["RHO_A"])
                h_adp = h_ia - (h_ia -h_oa)/adp_epsilon
                T_adp = self.get_T_adp_from_h_adp(h_adp,T_iw + 4)
                Q_sen = self._nominal_air_flow*self.props["RHO_A"]*self.props["C_PA"]*(T_idb-T_adp)*adp_epsilon
                if (Q_sen > Q_tot):
                    Q_sen = Q_tot
            if self.parameter("fan_operation").value == "CYCLING":
