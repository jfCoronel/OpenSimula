...
var DEBOUNCE_MS = 400;
var isList = (sch) => sch?.["x-list"] === true;
var itemSchema = (sch) => isList(sch) ? sch.anyOf[1] : sch;
...
var sameValue = (a, b) => JSON.stringify(a) === JSON.stringify(b);
var PLOTLY_URL = "https://esm.sh/plotly.js-gl3d-dist-min@3.7.0";
...
var column = (rows, index) => rows.map((row) => row[index]);
var HIGHLIGHT = "rgb(255, 127, 14)";
...
var isDefault = (sch, value) => value === void 0 || sameValue(value, sch.default);
function buildValidator(schema) {
  if (!schema || Object.keys(schema).length === 0) return {};
  try {
    const ajv = new import__.default({ strict: false, allErrors: true, discriminator: true });
...
var parseList = (kind, raw) => raw.split(",").map((part) => parseScalar(kind, part.trim())).filter(
  (v) => v !== ""
);
var formatList = (value) => Array.isArray(value) ? value.join(", ") : String(value ?? "");
var editor_src_default = {
  render({ model, el }) {
    ...
    let timer = null;
    let errorsByPath = /* @__PURE__ */ new Map();
    ...
    function refreshApply() {
      ...
      const blocked = errorsByPath.size > 0 && [...errorsByPath.values()].some((e) => e.severity === "error");
      applyButton.disabled = pending.length === 0 || blocked;
      applyButton.title = pending.length === 0 ? "Every change is already in the project" : pending.join("\n");
    }
    function refreshErrors() {
      const found = collectErrors();
      errorsByPath = /* @__PURE__ */ new Map();
    ...
    function renderTree() {
      ...
      const withErrors = /* @__PURE__ */ new Set();
    ...
    function makeInput(kind, sch, value, onCommit) {
      ...
      if (kind === "enum" && !isList(sch)) {
        ...
        for (const option of item.enum) {
          ...
          el2.value = el2.textContent = option;
          input.appendChild(el2);
        }
        input.value = value ?? "";
      ...
      if (kind === "ref" && !isList(sch)) {
        input = document.createElement("select");
        const allowed = item["x-ref"].allowed_types || [];
        const names = components().filter((c) => allowed.length === 0 || allowed.includes(c.type)).map((c) => c.name);
        if (value !== void 0 && !names.includes(value)) names.unshift(value);
        for (const name of names) {
          const el2 = document.createElement("option");
          el2.value = el2.textContent = name;
          input.appendChild(el2);
        }
        input.value = value ?? "";
        input.onchange = () => onCommit(input.value);
      ...
      if (isList(sch)) {
        ...
        if (item.minimum !== void 0) input.min = item.minimum;
        if (item.maximum !== void 0) input.max = item.maximum;
        input.value = value ?? "";
    ...
    function renderForm(container, sch, index) {
      ...
      title.textContent = index === null ? "Project parameters" : `${container.type}: ${container.name ?? ""}`;
    ...
    root.querySelector(".osm-new").onclick = () => {
      ...
      for (const [key, propSchema] of Object.entries(def.properties || {})) {
        ...
        if (propSchema.default !== void 0) created[key] = structuredClone(propSchema.default);
      }
    ...
    async function renderView() {
      ...
      viewInfos = infos;
      viewCount.textContent = ghost.length ? `${active.length} of ${geometry.meshes.length}` : `${active.length} surfaces`;
      try {
    ...
    function readCamera() {
      ...
      const live = scene._scene && typeof scene._scene.getCamera === "function" ? scene._scene.getCamera() : scene.camera;
