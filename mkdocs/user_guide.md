...
- **simulate(show_percentage = 10)**: Perform the time simulation of the project, calculating all the varibles of the components. Once the simulation has started, 
an informative progress message will be printed every time the percentage set in the _show_percentage_ parameter is exceeded (its default value is 10%),
and the average number of iterations performed in each time step, for that percentage, will also be displayed.
