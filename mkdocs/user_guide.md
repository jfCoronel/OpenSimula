...
- frequency [__None__, "hourly", "daily", "monthly", "yearly"]: Frequency of the data, that of the simulation (None), hourly, daily, monthly or yearly.
- value [__"mean"__,"max","min","sum","sum_pos","sum_neg"]: If we use a frequency other than the simulation frequency (e.g. "monthly"), the value obtained for each row (month) will be the mean ("mean"), the maximum ("max"), the minimum ("min"), the sum ("sum"), the sum of positive values ("sum_pos") or the sum of negative values ("sum_neg").
